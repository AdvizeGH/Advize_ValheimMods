...
public partial class Spyglass : BaseUnityPlugin
{
    ...
    private static Sprite CreateSprite(string fileName, Rect spriteSection)
    {
        ...
        if (cachedSprites.TryGetValue(texture, out Sprite cached))
            return cached;

        Sprite sprite = Sprite.Create(texture, spriteSection, Vector2.zero);
        cachedSprites[texture] = sprite;
        return sprite;
    }

    private static Texture2D LoadTexture(string fileName)
    {
        if (cachedTextures.TryGetValue(fileName, out Texture2D cached))
            return cached;

        using Stream stream = Assembly.GetExecutingAssembly().GetManifestResourceStream($"Advize_{PluginName}.Assets.{fileName}");

        byte[] data = new byte[stream.Length];
        stream.Read(data, 0, data.Length);

        Texture2D texture = new(0, 0);
        LoadImageMethod.Invoke(null, [texture, data]);
